> **文档状态**：📘 参考资料（外部，视频重绘线）
> **用途/说明**：火山方舟 Seedance 虚拟人像库说明（人物锚定参考）
> **权威来源**：本仓库权威文档为 `AGENTS.md` + `docs/local-life-user-journey.md`
> **最后校准**：2026-07-11

为保护个人隐私，Seedance 2.0 系列模型不支持直接上传含有真人人脸的参考图/视频。体验中心提供可用于 Seedance 2.0 视频生成的可信虚拟人像库，您可在体验中心浏览和检索虚拟人像，在视频生成中使用，结合模板和您的自有素材，快速验证视频效果。
同时，体验中心还提供类型丰富的视频生成模板，您查看视频生成使用的素材，并可一键获取模板同款示例代码。
> Tip：您可在[创建视频生成任务 API ](https://www.volcengine.com/docs/82379/1520757?lang=zh)中使用虚拟人生成视频，具体方式请参考[使用预置虚拟人像](/docs/82379/2291680#2bf01416)。

:::warning
* 火山方舟努力为用户呈现更多元化的内容。如您对本功能有任何意见或建议，欢迎提交[工单](https://console.volcengine.com/auth/login?redirectURI=%2Fworkorder%2Fcreate%3Fstep%3D2%26SubProductID%3DP00001166)或联系 Support 反馈。我们将仔细阅读和评估您的反馈，并尽快处理。
* 为确保合规使用本功能，请在使用前阅读[火山方舟体验中心服务规则和免责声明](https://www.volcengine.com/docs/82379/1108564?lang=zh)，并在[开通管理页](https://console.volcengine.com/ark/region:ark+cn-beijing/openManagement)完成开通。
   ![Image](https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/71a885da93494fffa0d5bca7b5a22b3f~tplv-goo7wpa0wc-image.image =2262x)


:::
<span id="901571f6"></span>
## 关于虚拟人像库
虚拟人像库提供了丰富的虚拟人像素材资源。每个虚拟形象由一张图片组成。可将虚拟人作为参考素材发送给模型，结合指令，让模型生成含有该形象的视频。
![Image](https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/06d28d889dd0494c9a1788deeb84fe1d~tplv-goo7wpa0wc-image.image =1444x)

每个虚拟人对应一个资产 ID。在使用视频生成 API 时，可使用含此 ID 的 URI 将人物素材传入模型。
平台还提供了丰富的非人像类素材，可在 **素材库** 查看其资产 ID 和 URI。
您可在体验中心检索虚拟人像素材、查看资产 ID，将其快速应用于视频生成任务中。
:::tip
* 当前暂不支持使用真实人物形象生成视频，建议使用平台提供的虚拟人像。
* 首次使用虚拟人像库时，需先同意相关协议，才可正常浏览和使用人像素材。
:::
<span aceTableMode="list" aceTableWidth="4,3,5"></span>
<span id="b2049b21"></span>
## 浏览和检索虚拟人像

1. 在浏览器中打开火山方舟[体验中心](https://console.volcengine.com/ark/region:ark+cn-beijing/experience/vision?modelId=doubao-seedance-2-0-260128&tab=GenVideo)，点击输入框下方的 **虚拟人像库** 页签。
2. 检索要使用的人像，支持自然语言检索和条件筛选。
   1. 自然语言检索示例：“积极阳光的青春男大，身高 180 cm 左右”
   2. 使用右侧筛选框，以人物性别、年龄、国籍维度进行筛选。 
3. 将鼠标悬停在人像上，点击 **查看虚拟人像详情** 可查看人物标签和小传。还可复制资产 ID 或 URI。 
   **注意**：要快速复制资产 ID，点击右上角的 **复制 asset ID**。 
4. 点击 **使用** 图标，可将人像添加至输入框。在输入框中使用 “@+素材名”格式引用素材，例如：

> ***@图片 1*** *中女主转身坐下，转镜头，女主下了一步棋子，并说“你输了”，参考* ***@图片 2*** *的分镜画面...*

:::tip
默认单次生成 4 段视频，为节约成本，建议设置为每次生成一段视频。
:::

5. 点击输入框右下角的发送图标，开始视频生成。


:::tip
Seedance 2.0 系列模型生成的视频为受信素材。您可使用**本账号下**由上述模型生成的视频，进行视频编辑、视频延长等二次创作，素材中的人脸可正常参与生成，不会触发审核拦截。
:::
<span id="58715d25"></span>
## 快速获取模板库示例代码
Seedance 2.0 体验中心提供视频生成模板库，每个模板包含完整指令和素材。您可使用模板一键生成，或基于模板，结合虚拟人像、自定义内容生成视频。
同时模板库支持查看模板的 REST API 调用示例。可直接复制示例命令进行测试。
参考以下步骤，快速查看示例代码：

1. 在 Seedance 2.0 体验中心界面，点击下方的 **模板库** 页签。
2. 将鼠标悬停在模板上，点击 **查看代码** 图标，即可查看和复制命令。

**注意**：测试前需先设置 `ARK_API_KEY` 环境变量，或将 `$ARK_API_KEY` 替换为您的 API Key。
<div style="text-align: center"><img src="https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/1344a21f888e4249b11b6c3694a094d3~tplv-goo7wpa0wc-image.image" width="580px" /></div>

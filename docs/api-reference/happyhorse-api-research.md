...
> **文档状态**：📘 参考资料（外部 API，视频重绘线）
> **用途/说明**：阿里云百炼 HappyHorse 视频生成模型 API 调研
...
> **重要**：方案 A（自动 fallback）违反项目「无静默降级」规则，已明确不采用。

~~**方案 A — 备选引擎(已否决)**~~:
- ~~自动 fallback~~ — 不允许。失败必须显式报错让用户知晓，不能静默切换引擎。

**方案 B — 独立模式（已采用）**:
- 用户在创建项目时手动选择引擎（`Project.engine`），Worker 按所选引擎调用对应 API
- HappyHorse 模式下链式续接改为 I2V(用上一组尾帧做 first_frame),连贯性比 reference_video 弱但可用
- 当前仅实现 V-Edit 单模式，T2V/I2V/R2V 后续按需扩展

---

## 八、接入所需改动(概要) — ⚠️ 部分已完成，以代码为准

> **实际实现路径**：`src/lib/shared/happyhorse.ts`（非文中建议的 `src/lib/happyhorse.ts`）
> **当前实现状态**：仅 V-Edit 模式已接入（`happyhorse-1.0-video-edit`），T2V/I2V/R2V 未实现。
> **引擎选择**：通过 `Project.engine` / `GenerationJob.engine` 字段实现显式用户选择，**不使用自动 fallback**（自动 fallback 违反项目「无静默降级」规则，见 AGENTS.md）。

1. ~~新增 `src/lib/happyhorse.ts`~~ → 实际位置 `src/lib/shared/happyhorse.ts`（已完成）
2. `generate-video.ts` — 已增加 `engine` 分支，**不做审核失败自动切换**，由用户显式选择引擎
3. 环境变量 — `DASHSCOPE_API_KEY`（已在 .env.example 中）
4. 数据库 — `Project.engine` + `GenerationJob.engine` 字段（已完成）

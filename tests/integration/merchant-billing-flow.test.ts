...
describe('商家计费渲染补偿集成测试（真实 PostgreSQL + Redis 锁）', () => {
...
describe('商家流水写入不触发 jobId 外键违约（Req 4.5，真实 PostgreSQL）', () => {
...
describe('各 Route 改造后走积分/权益，不再引用额度逻辑（Req 2.3，结构断言）', () => {
  ...
  it('计费 Route 改为引用统一积分/权益服务（merchant-billing-service / privilege-engine）', () => {
    ...
    // 渲染路由走统一积分计费封装
    expect(/from\s+['"]@\/lib\/merchant-billing-service['"]/.test(renderSrc)).toBe(true)
    ...
    // 洞察路由走统一权益引擎
    expect(/from\s+['"]@\/lib\/privilege-engine['"]/.test(insightsSrc)).toBe(true)
  })
})

describe('同一 JWT 会话访问 /merchant 与 /dashboard 均放行（Req 8.2，中间件配置断言）', () => {
...
describe('既有 Merchant/Store/ContentBrief/VideoVariant 读写正常（Req 7.1，真实 PostgreSQL）', () => {

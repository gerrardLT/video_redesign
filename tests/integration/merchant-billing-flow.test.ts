...
describe.skipIf(skipIfNoInfra)('同一 JWT 会话访问 /merchant 与 /admin 均放行（Req 8.2，中间件配置断言）', () => {
  // Req 8.2：已登录会话在本地生活主框架（/merchant）与管理后台（/admin）等受保护页面间导航无需重新认证。
  //（原视频重塑模块 /dashboard 页面已并入 /merchant，跨店看板改由 GET /api/stores/dashboard 提供数据，
  //  已不存在 /dashboard 页面前缀；故受保护页面前缀现为 /admin 与 /merchant。）
  ...
  it('PROTECTED_PAGE_PREFIXES 同时覆盖 /admin 与 /merchant', () => {
  ...
  it('matcher 配置同时匹配 /admin 与 /merchant', () => {
  ...
  it('两类受保护页面共用同一套 x-user-id / x-user-role 注入逻辑', () => {
    ...
    // 说明 /merchant 与 /admin（同属页面分支）经同一 JWT 校验+注入路径放行。

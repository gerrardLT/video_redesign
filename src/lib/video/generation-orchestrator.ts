...
import { segmentVideo, cutVideoSegments, type VideoSegment } from '@/lib/video/segment-service'
import { downloadToTemp } from '@/lib/shared/storage'
...
export async function orchestrateHappyHorseGeneration(
  ...
  if (isDirect) {
    ...
    try {
      ...
      // 上传各段到 OSS 并收集 URL
      const { uploadFile, getPublicUrl } = await import('@/lib/shared/storage')

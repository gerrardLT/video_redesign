...
import { getPublicUrl, isOSSConfigured, getSignedObjectUrl } from '../shared/storage'
import {
...
import { buildMerchantContextByUserId } from '../merchant/merchant-context-builder'
...
export interface GroupGenReference {
  ...
  /** 商家画像前缀（仅 merchant 用户有门店时非空，拼到 prompt 前让 Seedance 更懂商家） */
...
export async function buildGroupGenReference(shotGroupId: string): Promise<GroupGenReference> {
  ...
  try {
    ...
    if (projectUser?.userId) {
      const merchantCtx = await buildMerchantContextByUserId(projectUser.userId)
      if (merchantCtx?.promptPrefix) {
        merchantPrefix = merchantCtx.promptPrefix
      }
    }
  } catch (err) {
    // 非关键路径：商家画像查询失败不阻塞生成，仅记录警告
    console.warn('[group-gen-context] 查询商家画像上下文失败（不影响主流程）:',

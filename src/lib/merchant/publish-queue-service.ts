...
import type { PublishQueueItem, ContentBriefStatus } from '@/generated/prisma'
import { canBriefTransition } from './content-brief-state-machine'
...
export async function enqueueForPublish(input: {
  ...
  // ─── Step 3: 原子幂等入列（Property 29：一一对应）───
  // 使用 upsert + @@unique([videoVariantId]) 保证并发安全：
  // 并发调用时仅一方 create 成功，其余走 update（无实际变更，返回既有记录）。
  return prisma.publishQueueItem.upsert({
 ...
 * 数据回填/复盘范围（可反哺）。
 *
 * 状态同步：首次标记发布时，若 ContentBrief 仍为 EXPORTED 状态，
 * 自动同步更新为 PUBLISHED（状态机守卫：仅 EXPORTED → PUBLISHED 合法转换时执行）。
 *
...
export async function markPublished(input: {
  ...
  // ─── Step 1: 加载现有清单项（含关联 contentBriefId）───
  const item = await prisma.publishQueueItem.findUnique({
    where: { id: publishQueueItemId },
    select: { publishedPlatforms: true, contentBriefId: true },
  })
  if (!item) {
    throw new PublishQueueError(
  ...
  // ─── Step 5: 同步 ContentBrief 状态（EXPORTED → PUBLISHED）───
  // 首次标记发布时，若 brief 仍为 EXPORTED 且状态机允许转换，自动同步。
  // 已发布/已归档的 brief 不重复更新（守卫保证幂等）。
  const brief = await prisma.contentBrief.findUnique({
    where: { id: item.contentBriefId },
    select: { status: true },
  })
  ...
  if (
    ...
    canBriefTransition(brief.status as ContentBriefStatus, 'PUBLISHED')
  ) {

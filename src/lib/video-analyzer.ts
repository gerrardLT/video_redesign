...
6. dialogue 必须准确转录视频音频中听到的对白，无对白则留空数组
...
【字数铁律 — 极其重要】：
单个分镜 suggestedPrompt 必须控制在 **40-60 字**以内（含时间码和对白）。
原因：多个分镜会合并成一条提交给 Seedance，整条总长须 ≤250 字，否则模型会忽略后半段（开始"取平均"产出通用结果）。
因此每段只保留：1 个运镜 + 1 个核心物理化动作 + 1 个光线/氛围关键词（+ 对白）。

...
export interface DownloadVideoJobData {
  ...
  // 项目名称是否为系统自动生成（用户未填写）：true 时下载完成后用 yt-dlp 真实标题回写 name
...
async function processDownloadVideo(job: Job<DownloadVideoJobData>): Promise<void> {
  const { taskId, projectId, sourceUrl, platform, nameAutoGenerated } = job.data
  ...
  try {
    ...
    //    若名称为系统自动生成（用户未填写），同时把 yt-dlp 真实标题归一化后回写为项目名称。
    //    复用同一次 update，不新增数据库往返；用户已填写名称时完全不触碰 name。
    await prisma.project.update({
      where: { id: projectId },
      data: buildProjectUpdateOnDownloadComplete({
        ossUrl,
        nameAutoGenerated,
        title,
        platform,

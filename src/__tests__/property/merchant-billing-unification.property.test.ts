...
// PostgreSQL 连接检测：CI 环境无数据库时自动跳过依赖 DB 的属性测试
...
try {
  ...
  pgAvailable = false
  console.warn('⚠️ PostgreSQL 不可用，跳过依赖数据库的属性测试: merchant-billing-unification.property.test.ts')
}
...
/** 依赖 Redis 与 PostgreSQL 的属性测试需实际连接（withCreditLock + Prisma 事务） */
const skipIfNoRedis = !process.env.REDIS_URL || !pgAvailable
...
describe.skipIf(!pgAvailable)('Property 3: 无扣减操作余额守恒', () => {
